...
Процедура ПроверитьНаСервере()
	...
	Схема22 = Новый СхемаЗапроса;
	Схема22.УстановитьТекстЗапроса(ТекстЗапроса);
	КэшСтруктурыМетаданных = Неопределено;
	РезультатПроверки = ПустаяТаблицаРезультатов();
	
	ПроверитьСхемуНаПападаниеУсловийВИндексы(Схема22,РезультатПроверки, КэшСтруктурыМетаданных);
	ПроверитьСхемуНаИспользованиеИЛИ(Схема22,РезультатПроверки);
	ПроверитьСхемуНаРаботуСВиртуальнойТаблицей(Схема22,РезультатПроверки);
	
	ТекстЗапроса = ТекстЗапросаСРезультатомПроверок(Схема22, РезультатПроверки);
